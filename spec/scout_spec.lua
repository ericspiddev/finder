...
describe("Scout", function()
    ...
    it('sets the width and logger level based on the passed in config', function()
        ...
        local test_scout_config = {
            debug_level = logger.DEBUG_LEVELS.INFO,
        ...
        assert.equals(Scout_Logger.log_level, logger.DEBUG_LEVELS.INFO)
        test_scout.search_bar:close()

        test_scout_config.debug_level = logger.DEBUG_LEVELS.ERROR
        test_scout_config.width_percentage = 0.35
        test_scout.setup(test_scout_config)
        test_scout.search_bar:open()
        assert.equals(test_scout.search_bar.query_win_config.width, math.floor(250 * 0.35))
        assert.equals(Scout_Logger.log_level, logger.DEBUG_LEVELS.ERROR)
        test_scout.search_bar:close()

        test_scout_config.debug_level = logger.DEBUG_LEVELS.DEBUG
        test_scout_config.width_percentage = 0.635
        test_scout.setup(test_scout_config)
        test_scout.search_bar:open()
        assert.equals(test_scout.search_bar.query_win_config.width, math.floor(250 * 0.635))
        assert.equals(Scout_Logger.log_level, logger.DEBUG_LEVELS.DEBUG)
        test_scout.search_bar:close()

        test_scout_config.debug_level = logger.DEBUG_LEVELS.WARNING
        ...
        assert.equals(Scout_Logger.log_level, logger.DEBUG_LEVELS.WARNING)
        test_scout.search_bar:close()

        test_scout_config.debug_level = logger.DEBUG_LEVELS.OFF
        test_scout_config.width_percentage = 0.95
        test_scout.setup(test_scout_config)
        test_scout.search_bar:open()
        assert.equals(test_scout.search_bar.query_win_config.width, math.floor(250 * 0.95))
        assert.equals(Scout_Logger.log_level, logger.DEBUG_LEVELS.OFF)
        test_scout.search_bar:close()
