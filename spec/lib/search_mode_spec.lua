local mode = require("lib.search_mode")
...
describe('search mode', function ()
    ...
    it('removes all spaces in the name for hl_name', function ()
        local m = mode:new("Eric Mode", "E", 0, "Red")
        assert.equals(m.hl_name, "EricMode")
        m = mode:new("Much longer name of string", "m", 0, "blue")
        assert.equals(m.hl_name, "Muchlongernameofstring")
        m = mode:new("whoknows what this mode is", "w", 0, "green")
        assert.equals(m.hl_name, "whoknowswhatthismodeis")
    end)
