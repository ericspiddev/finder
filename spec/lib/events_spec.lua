...
describe('events', function()
    ...
    it('identifies valid and invalid events', function()
        ...
        utils:finder_print_was_called(debug_levels.ERROR, "Nil valid events table no events will be allowed")
        ...
        assert(not test_events:is_valid_event("on_lines"))
        utils:finder_print_was_called(debug_levels.ERROR, "Unsupported event ", "on_lines")
        assert(not test_events:is_valid_event(""))
        utils:finder_print_was_called(debug_levels.ERROR, "Unsupported event ", "")
        assert(not test_events:is_valid_event())
        ...
        utils:finder_print_was_called(debug_levels.ERROR, "Unsupported event ", "s0me_change_occurred")
        assert(not test_events:is_valid_event("te3st"))
        utils:finder_print_was_called(debug_levels.ERROR, "Unsupported event ", "te3st")
        assert(not test_events:is_valid_event("idk"))
        utils:finder_print_was_called(debug_levels.ERROR, "Unsupported event ", "idk")
        assert(not test_events:is_valid_event(""))
        utils:finder_print_was_called(debug_levels.ERROR, "Unsupported event ", "")
        assert(not test_events:is_valid_event(nil))
        assert(not test_events:is_valid_event(20))
        utils:finder_print_was_called(debug_levels.ERROR, "Unsupported event ", 20)
        assert(not test_events:is_valid_event())
    ...
    it('registers handlers for valid events and blocks it for invalid ones', function()
        ...
        utils:finder_print_was_called(debug_levels.ERROR, "Unsupported event supported events are ", fake_valid_events)
    ...
    it('does not register nil handlers', function()
        ...
        utils:finder_print_was_called(debug_levels.ERROR, "Nil event handler for event ", "valid_event")
    end)
    ...
    it('does not register on a nil instance', function()
        ...
        assert(test_events.event_table['valid_event'] == nil)
        utils:finder_print_was_called(debug_levels.ERROR, "Nil object instance")
    end)
    ...
    it('handles an invalid buffer when attaching events', function()
        ...
        utils:finder_print_was_called(debug_levels.ERROR, "Cannot attach to invalid nvim buffer ", fake_buffer)
    end)
    ...
    it('handles an empty event_table when attaching events', function()
        ...
        assert(not test_events:attach_buffer_events(fake_buffer))
        utils:finder_print_was_called(debug_levels.ERROR, "Failed to attach buffer events empty or nil event table!")
    ...
    it('handles a failed event attach on a buffer', function()
        ...
        assert(not test_events:attach_buffer_events(fake_buffer))
        utils:finder_print_was_called(debug_levels.ERROR, "Failed to attach events to buffer ", fake_buffer)
    end)
    ...
    it('can attach events to a buffer', function()
        ...
        utils:finder_print_was_called(debug_levels.INFO, "Successfully attached buffer events")
    end)
